...
const SETTINGS_PATH := "res://Configs/devtools_settings.json"
...
var _tabs_focus_backup := Control.FOCUS_ALL
var _selector_disabled_backup := false
var _listen_button_disabled_backup := false
var _clear_button_disabled_backup := false
var _reset_button_disabled_backup := false
var _invert_toggle_disabled_backup := false
var _repair_bindings_button_disabled_backup := false
var _listen_button_default_text := ""
...
func _hook_input_bootstrap() -> void:
	...
	var toggle_callable := Callable(self, "_on_devtools_toggle_requested")
	...
	var actions_callable := Callable(self, "_on_input_actions_ready")
...
func _collect_tank() -> void:
	if get_tree() == null:
		return  # DevTools n'est plus dans une scène : éviter crash silencieux
	...
	while _tank == null and attempts < 240:
		...
		var tanks := tree.get_nodes_in_group("tank")
		if tanks.size() > 0:
...
func _connect_signals() -> void:
	if _tank:
		var c1 := Callable(self, "_on_tank_gear_changed")
		if not _tank.is_connected("gear_changed", c1):
			_tank.connect("gear_changed", c1)

		var c2 := Callable(self, "_on_tank_moved")
		if not _tank.is_connected("moved", c2):
			_tank.connect("moved", c2)

		var c3 := Callable(self, "_on_tank_action")
		if not _tank.is_connected("action_performed", c3):
			_tank.connect("action_performed", c3)

		var c4 := Callable(self, "_on_tank_tree_exited")
		if not _tank.is_connected("tree_exited", c4):
			_tank.connect("tree_exited", c4)

	if _stage_manager:
		var loaded := Callable(self, "_on_stage_loaded")
		if _stage_manager.has_signal("stage_loaded") and not _stage_manager.is_connected("stage_loaded", loaded):
			_stage_manager.connect("stage_loaded", loaded)

		var unloaded := Callable(self, "_on_stage_unloaded")
		if _stage_manager.has_signal("stage_unloaded") and not _stage_manager.is_connected("stage_unloaded", unloaded):
			_stage_manager.connect("stage_unloaded", unloaded)

		var completed := Callable(self, "_on_stage_completed")
		if _stage_manager.has_signal("stage_completed") and not _stage_manager.is_connected("stage_completed", completed):
			_stage_manager.connect("stage_completed", completed)

	if _mother_ai:
		var tstart := Callable(self, "_on_mother_ai_task_started")
		if _mother_ai.has_signal("task_started") and not _mother_ai.is_connected("task_started", tstart):
			_mother_ai.connect("task_started", tstart)

		var tdone := Callable(self, "_on_mother_ai_task_completed")
		if _mother_ai.has_signal("task_completed") and not _mother_ai.is_connected("task_completed", tdone):
			_mother_ai.connect("task_completed", tdone)

		var logc := Callable(self, "_on_mother_ai_log_updated")
		if _mother_ai.has_signal("log_updated") and not _mother_ai.is_connected("log_updated", logc):
			_mother_ai.connect("log_updated", logc)
...
func _register_mechanic_control(property_name: String, slider: HSlider, label: Label, format: String) -> void:
	...
	slider.value_changed.connect(
		Callable(self, "_on_mechanic_slider_value_changed").bind(property_name, label, format)
	)
...
# =======================================================================================
# PERSISTENCE (save / load DevTools settings)
# =======================================================================================
...
func _load_saved_settings() -> void:
	...
	if not FileAccess.file_exists(SETTINGS_PATH):
		print("🕸️ LE fichier n'existe pas à l'adresse" + (SETTINGS_PATH))
		return
...
func _apply_saved_settings() -> void:
	if _tank == null or _saved_settings.is_empty() :
		return
...
func _on_reset_binding_pressed() -> void:
	...
	if _input_bootstrap and _input_bootstrap.has_method("reset_action"):
		# reset_action ne retourne rien : on assume succès
		_input_bootstrap.reset_action(action_name)
		var saved := _save_bindings()
...
func _on_repair_bindings_pressed() -> void:
	if _input_bootstrap and _input_bootstrap.has_method("reset_all"):
		_input_bootstrap.reset_all()
		var saved := _save_bindings()

		if _binding_status:
			_binding_status.text = "All bindings reset to defaults."
			if not saved:
				_binding_status.text += " [Save failed.]"
	else:
		if _binding_status:
			_binding_status.text = "Repair unavailable: reset_all() missing."

	var action_name := _get_selected_action()
	if action_name != "":
		_refresh_binding_display(action_name)
...
func _save_bindings() -> bool:
	if _input_bootstrap and _input_bootstrap.has_method("save_user_bindings"):
...
func _on_save_input_bindings_pressed() -> void:
	...
	if _binding_status:
		_binding_status.text = (
			"Bindings saved to disk."
			if saved
			else "Cannot save: save_user_bindings() failed."
		)



# ==========================================================
# Listen handler / input capture
# ==========================================================

func _get_selected_action() -> String:
	if _input_action_selector == null or _input_action_selector.item_count == 0:
		return ""
	var index := _input_action_selector.get_selected_id()
	if index < 0:
		index = 0
...
func _format_input_event(event: InputEvent) -> String:
	if event is InputEventKey:
		...
		return "Joy Axis %d (%.1f)" % [event.axis, event.axis_value]
	return "Unknown"


func _unhandled_input(event: InputEvent) -> void:
	
	# ========== DevTools menu toggle ==========
	...
	if event is InputEventKey:
		...
		accepted = absf(event.axis_value) >= 0.5
	...
	if _input_bootstrap and _input_bootstrap.has_method("remap_action"):
		...
		# fallback minimal
		InputMap.action_erase_events(String(action_name))
		InputMap.action_add_event(String(action_name), new_event)
	...
	if _binding_status:
		_binding_status.text = "Assigned %s to %s." % [
			_format_input_event(new_event),
			action_name
		]
		if not saved:
			_binding_status.text += " [Save failed.]"
...
func _update_metrics_overlay() -> void:
	...
	#_metrics_overlay.text = "FPS: %.0f | Stage: %s | Sandbox: %s" % [
	#	fps,
	#	stage_text,
	#	"ON" if _sandbox_enabled else "OFF"
	#]
