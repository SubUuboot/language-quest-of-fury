...
func _on_clear_binding_pressed() -> void:
	...
	if _input_bootstrap and _input_bootstrap.has_method("save_user_bindings"):
		saved = _input_bootstrap.save_user_bindings()
...
func save_user_binding() -> bool:
	if _input_bootstrap and _input_bootstrap.has_method("save_user_binding"):
		return _input_bootstrap.save_user_binding()
	return false


func _on_save_input_bindings_pressed() -> void:
	if _input_bootstrap and _input_bootstrap.has_method("save_user_binding"):
		_input_bootstrap.save_user_binding()
		if _binding_status:
			_binding_status.text = "Bindings saved to disk."
	else:
		if _binding_status:
			_binding_status.text = "Cannot save: InputBootstrap.save_user_binding() missing."
...
func _format_input_event(event: InputEvent) -> String:
	if event is InputEventKey:
		...
		return "Joy Button %d" % b.button_index
	elif event is InputEventJoypadMotion:
		var m: InputEventJoypadMotion = event
		return "Joy Axis %d (%.1f)" % [m.axis, m.axis_value]
	return "Unknown"
